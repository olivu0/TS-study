...
// ++
function add5(x: number, y: number): number; // type만 정의
// type 빼고 선언만
function add5(x, y) {
  return x + y;
}
...
aa = 'hello' as unknown as number; // error 사라짐 + as 뒷부분 모두 사라짐

// never 타입과 느낌표(non-null assertion)
try {
  // const array = []; // type: never
  // array[0];
  const array: string[] = []; // 빈배열을 선언할 때는 반드시 타이핑 지정하기
