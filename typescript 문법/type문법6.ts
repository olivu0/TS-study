...
const xx6: object = 'hi'; // 이게 진짜 객체
const yy6: object = { hello: 'world' };
const z6: unknown = 'hi';
...
const aaaa: A6 = { a: 'hello', b: 'world' };
// 실수로 바꾸려는 것을 못바꾸게 막아줌
aaaa.a = '123'; // 이렇게 바꿀 수 있음 (js)-> 읽기 전용으로 못바꾸게 함(ts)

// 인덱스드 시그니처
// 전부 다 문자열이었으면 좋겠는데 속성이 너무 많음
type B6 = {
  // [key: string]: string; // 어떤 key이던 key도 문자열, 값도 문자열
  [key: string]: number; // 어떤 key이던 key도 문자열, 값은 숫자
};
const bbbb: B6 = { a: 3, b: 4 };

// 맵드 타입스 (key를 줄이는 것) - 제한을 걸어두는 것
type C6 = 'Human' | 'Mammal' | 'Animal';
type D6 = { [key in C6]: number };
type F6 = { [key in C6]: C6 };
const dddd: D6 = { Human: 5, Mammal: 6, Animal: 4 };
const ffff: F6 = { Human: 'Animal', Mammal: 'Mammal', Animal: 'Human' };

// 클래스의 새로운 기능들
class AB {
  a: string;
  b: number;
  #c: number = 123; // js private (protected 안됨)
  // ts에서의 private을 실제 코드로 바꾸면 public으로 바뀜(바뀌기 전 정교함)
  private d: string = '123'; // ts private(이거 쓰는게 낫다)
...
const ab: AB = new AB('123');

// 클래스 자체를 가리키는 것 = 클래스 자체의 타입
const abab: typeof AB = AB;

// 클래스에 추가된 것들
// interface, implements는 js로 변환시 사라짐
interface AAA {
  readonly a: string;
  b: string;
}
...
class BBB implements AAA {
  private readonly a: string = '123'; // 여러개 복합해서 쓰는 것 가능
  protected b: string = 'world';
  c: string = 'wow'; // public인데 이건 기본이여서 안써도 됨
