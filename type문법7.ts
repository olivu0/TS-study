...
let obj7: { a: string; b?: string } = { a: 'hello', b: 'world' };
obj7 = { a: 'hello' };
...
// T의 타입을 string으로 제한함
function addGe<T extends string>(x: T, y: T): T {
  return x + y;
}
...
function addGen<T extends string | number>(x: T, y: T): T {
  return x + y;
}
...
function addGene<T extends number, K extends string>(x: T, y: K): T {
  return x + y;
}
...
function addGener<T extends { a: string }>(x: T): T {
  return x;
}
addGener({ a: 'hello' });

function addGeneri<T extends string[]>(x: T): T {
  return x;
}
addGeneri(['1', '2', '3']);

// 콜백함수의 형태로 제한
function addGeneric<T extends (a: string) => number>(x: T): T {
  return x;
}
addGeneric((a) => +a); // a: string, +a : number

// 모든 함수 가능 -> 제한걸때는 any 써도 됨
function addGenericAny<T extends (...args: any) => any>(x: T): T {
  return x;
}

function addGenericConstructor<T extends abstract new (...args: any) => any>(x: T): T {
  return x;
}
// 클래스 자체를 넣고 싶을 때
class Aconstructor {} // 생성자
...
const basic = (b: number = 3, c: number = 5) => {
  return '3';
};

const basicObj = (b: { children: string } = { children: 'olivu0' }) => {
  return;
};

// react -> jsx에서는 <T>부분이 error가 남, <div>같은 거랑 헷갈려서
const basicGeneric = <T>(x: T, y: T) => ({ x, y });

// react일때는
const reactGeneric = <T = unknown>(x: T, y: T) => ({ x, y });
const reactGeneric2 = <T extends unknown>(x: T, y: T) => ({ x, y });
const reactGeneric3 = <T>(x: T, y: T) => ({ x, y });
